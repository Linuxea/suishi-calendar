...
- **已部署**：自有服务器的 `/calendar/` 子路径，`vite.config.ts` 的 `base` 必须保持 `'/calendar/'`。更新流程：`npm run build` → 将 `dist/` rsync 到服务器站点目录。服务器地址、ssh 别名、nginx 路径等敏感信息只记录在本地工作区 AGENTS.md（不入 git 仓库），**不要写入本仓库**
